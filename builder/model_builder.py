...
class ModelBuilder:
    ...
    def build(
        ...
        label.items = list(configuration.items)

        # -------------------------------------------------
        # Операционная система
        # -------------------------------------------------

        label.operating_system = configuration.operating_system
        ...
        storage_items = configuration.get_items(Category.STORAGE)

        storage_parts = []

        for storage_item in storage_items:

            storage_name = self.storage_builder.build(storage_item.name)
            ...
            if storage_item.quantity > 1:
                storage_name = (
                    f"{storage_name} x{storage_item.quantity}"
                )

            storage_parts.append(storage_name)

        label.storage = "/".join(storage_parts)
        ...
        if gpu:
            label.gpu = self.gpu_builder.build(gpu[0].name)
        ...
        if case:
            label.case = self.case_builder.build(case[0].name)
        ...
        if psu:
            label.psu = self.psu_builder.build(psu[0].name)
